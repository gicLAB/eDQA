...
from models_with_algorithm.mobilenetv2_pad_saw_compress_abl import MobileNetV2
import argparse
import copy
import numpy as np
import pickle
...
if args.model == 'resnet32':
    ...
    model = MobileNetV2(bit=args.bit, m=args.m, imp_ratio=args.imp_ratio, comp=args.comp, seed=args.seed)
    model = nn.DataParallel(model, device_ids=args.gpu).cuda()
...
'''
with open('experiment_res/abl_compress_verify/DQA_res_abl_' + str(args.model) +'_m_' + str(args.m) +'_imp_'+str(int(args.imp_ratio * 100))+'_comp_'+str(args.comp)+'_seed_' + str(args.seed) + '_bit_' + str(args.bit) + '.pkl', 'wb') as file:
