...
import models_with_algorithm.resnet_pad_saw_compress_abl as resnet
import models_with_algorithm.mobilenetv2_pad_saw_compression_ablation_ext as mobilenet
import models_with_algorithm.res_18_pad_saw_compression_ablation_ext as res_18
import models_with_algorithm.vision_transformer_pad_saw_compression_ablation as vit
...
parser.add_argument('--comp', type=str, default='huffman')
...
if args.model == 'resnet32':
    model = resnet.resnet32(bit=args.bit, m=args.m, imp_ratio=args.imp_ratio, comp=args.comp, seed=args.seed)
    ...
    model = mobilenet.MobileNetV2(bit=args.bit, m=args.m, imp_ratio=args.imp_ratio, comp=args.comp, seed=args.seed)
    model.linear = nn.Linear(model.linear.in_features, 200)
    model = nn.DataParallel(model, device_ids=args.gpu).cuda()
    checkpoint = '/home/wenhao/DQA_Exp_Ext/train_model/mobilev2_tinyimagenet_50_epochs.pth'
elif args.model == 'vit_im':
    model = vit.vit_b_16(bit=args.bit, m=args.m, imp_ratio=args.imp_ratio, comp=args.comp, seed=args.seed)
    ...
    model = res_18.resnet18(bit=args.bit, m=args.m, imp_ratio=args.imp_ratio, comp=args.comp, seed=args.seed)
...
loss, acc_post_val, avg_time, avg_mem, avg_act, avg_overhead, avg_hc, avg_ht, avg_ucu = validate(val_loader, model, L_cls_f, 'test')
...
with open('experiment_res/abl_compress/DQA_res_abl_' + str(args.model) +'_m_' + str(args.m) +'_imp_'+str(int(args.imp_ratio * 100))+'_comp_'+str(args.comp)+'_seed_' + str(args.seed) + '_bit_' + str(args.bit) + '_ext.pkl', 'wb') as file:
