...
import models_with_algorithm.resnet_pad_saw_dist as resnet
import models_with_algorithm.mobilenetv2_pad_saw_dist as mobilenet
import models_with_algorithm.res_18_pad_saw_dist as res_18
import models_with_algorithm.vision_transformer_pad_saw_dist as vit
...
def validate(val_loader, model, L_cls_f, loader_type=''):
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            ...
            a_dist = model.module.get_mem_usage()
            history_dist.append(a_dist)
...
if args.model == 'resnet32_cifar':
    ...
    model = mobilenet.MobileNetV2(bit=args.bit, m=args.m, imp_ratio=args.imp_ratio, data='cifar', seed=args.seed)
    model = nn.DataParallel(model, device_ids=args.gpu).cuda()
    ...
    model = mobilenet.MobileNetV2(bit=args.bit, m=args.m, imp_ratio=args.imp_ratio, data='im', seed=args.seed)
    model.linear = nn.Linear(model.linear.in_features, 200)
    model = nn.DataParallel(model, device_ids=args.gpu).cuda()
...
res = validate(val_loader, model, L_cls_f, 'test')

print(res)


with open('experiment_res/DQA_res_abl_' + str(args.model) + '_bit_' + str(args.bit) + '_dist.pkl', 'wb') as file:
