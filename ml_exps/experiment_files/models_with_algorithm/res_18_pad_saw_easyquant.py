...
class ResNet(nn.Module):
    def __init__(
        ...
        self.layer1 = self._make_layer(N, block, 64, layers[0])
        self.layer2 = self._make_layer(N, block, 128, layers[1], stride=2, dilate=replace_stride_with_dilation[0])
        self.layer3 = self._make_layer(N, block, 256, layers[2], stride=2, dilate=replace_stride_with_dilation[1])
        self.layer4 = self._make_layer(N, block, 512, layers[3], stride=2, dilate=replace_stride_with_dilation[2])
    ...
    def _make_layer(
        ...
        layers.append(
            block(
                N, False, self.inplanes, planes, stride, downsample, self.groups, self.base_width, previous_dilation, norm_layer
            )
        )
        count+=1
