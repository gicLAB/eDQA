...
std::vector<float> load_golden_data(int num_channels, int channel_size) {
  std::string filename = "data/" + std::to_string(num_channels) + "_" +
                         std::to_string(channel_size) + "_golden_noisy.csv";
  return load_data(filename);
}
...
int main() {
  ...
  // std::vector<int> channel_sizes = {10, 49, 196, 784, 3136};
  // std::vector<int> channel_sizes = {49};
  // std::vector<int> channel_sizes = {10,10}; // Example sizes

  // std::vector<int> dims = {7};
  std::vector<int> dims = {7, 14, 28, 56};

  std::vector<int> channel_sizes;

  for (int dim : dims) channel_sizes.push_back(dim * dim);

  // Save experiment data to CSV
  std::string csv_filename = "dqa_noisy_experiment_results.csv";
  ...
  for (int i = 0; i < channel_sizes.size(); i++) {
    ...
    float send_time_mean = total_send_time / Num_runs;
    float recv_time_mean = total_recv_time / Num_runs;
    for (int run = 0; run < Num_runs; ++run) {
      ...
      dequantize_cycles_stdev +=
          ...
          (dequantize_cycles_values[run] - dequantize_mean);
      send_time_stdev += (send_time_values[run] - send_time_mean) *
                         (send_time_values[run] - send_time_mean);
      recv_time_stdev += (recv_time_values[run] - recv_time_mean) *
                         (recv_time_values[run] - recv_time_mean);
    }
    ...
    string run_times_csv_filename =
        "noisyquant_run_times_" + std::to_string(channel_size) + ".csv";
